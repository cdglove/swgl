...
static void line(Vec2i p0, Vec2i p1, swgl::image& rt, TGAColor color) {
  bool steep = false;
...
static void triangle(Vec2i t0, Vec2i t1, Vec2i t2, swgl::image& rt, TGAColor color) {
  ...
  for(int i = 0; i < total_height; i++) {
    ...
    for(int j = A.x; j <= B.x; j++) {
      rt.set(j, t0.y + i, color); // attention, due to int casts t0.y+i != A.y
    }
  }
}
...
void pipeline::draw(model const& model, image& rt) const {
  ...
  for(int i = 0; i < model.nfaces(); i++) {
    ...
    if(intensity > 0) {
      triangle(
          screen_coords[0], screen_coords[1], screen_coords[2], rt,
