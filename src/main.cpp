...
int main(int argc, char** argv) {
    ...
    for (int i=0; i<model->nfaces(); i++) {
        ...
        for (int j=0; j<3; j++) {
            ...
            position[j] = vector2i((v.x+1.)*width/2., (v.y+1.)*height/2.);
            world_coords[j]  = v;
        }
        ...
        if (intensity>0) {
            triangle(position[0], position[1], position[2], image, TGAColor(intensity*255, intensity*255, intensity*255, 255));
        }
    }
